...
def ingest_email(event, context=None):
    ...
    if context != "gmail_api" or not isinstance(event, dict) or "payload" not in event:
        logger.info("Using test data for email ingestion")
        normalized = {
            "from": "test@domain.com",
            "subject": "Test",
            "body": "Hello world",
        }
        logger.info("Ingested: %s", normalized)
        return normalized
    ...
    try:
        ...
        logger.info("Ingested (error fallback): %s", normalized)
        return normalized
...
def decode_base64_body(data):
    ...
    try:
        ...
        logger.error("Error decoding body: %s", e)
        return ""
